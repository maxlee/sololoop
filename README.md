...
iteration: 1
max_iterations: 10
completion_promise: "DONE"  # v6 默认值
openspec_mode: true
...
v6 采用 `systemMessage` 分离迭代信息，保持 prompt 纯净：
...
├── commands/
│   ├── sololoop.md          # 纯循环命令
│   ├── openspec.md          # OpenSpec 桥接命令（v6: + 触发符）
│   └── cancel-sololoop.md   # 取消命令
├── hooks/
│   ├── hooks.json           # Hook 配置
│   └── stop-hook.sh         # Stop Hook 脚本（v6: Promise 驱动）
├── scripts/
│   ├── setup-sololoop.sh    # 初始化脚本
│   ├── openspec-bridge.sh   # OpenSpec 桥接脚本（v6: + 列表功能）
│   └── cancel-sololoop.sh   # 取消脚本
├── tests/                   # 测试文件
│   ├── setup-sololoop.bats
│   ├── openspec-bridge.bats # v6 更新
│   ├── stop-hook.bats       # v6 更新
│   ├── cancel-sololoop.bats
│   ├── command-format.bats
│   ├── integration.bats     # v6 更新
│   └── helpers/
└── README.md
